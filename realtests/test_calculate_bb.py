...
class TestCalculateBb:
    ...
    def test_bb_defaults(self):
        """Verify BB(20,2.0) for AAPL returns ordered bands."""
        u, m, l = calculate_bb("AAPL")
        assert u.iloc[-1] > m.iloc[-1] > l.iloc[-1]
        assert l.iloc[-1] > 0.0

    def test_bb_window_5(self):
        """Verify BB(5,2.0) for MSFT returns ordered bands."""
        u, m, l = calculate_bb("MSFT", window=5, num_std=2.0)
        assert u.iloc[-1] > m.iloc[-1] > l.iloc[-1]

    def test_bb_window_14(self):
        """Verify BB(14,2.0) for GOOG returns ordered bands."""
        u, m, l = calculate_bb("GOOG", window=14, num_std=2.0)
        assert u.iloc[-1] > m.iloc[-1] > l.iloc[-1]

    def test_bb_with_weekly_interval(self):
        """Verify BB works with a weekly bar interval."""
        u, m, l = calculate_bb("AAPL", window=10, num_std=2.0,
                                interval="1wk")
        assert u.iloc[-1] > m.iloc[-1] > l.iloc[-1]
    ...
    def test_bb_window_one(self):
        ...
        price (std=0)."""
        u, m, l = calculate_bb("AAPL", window=1, num_std=2.0)
        assert u.iloc[-1] == m.iloc[-1] == l.iloc[-1]
